...
describe("Kitapsepeti Login Testleri", () => {
   ...
   it("TC02_Login Form alanı kontrolü", () => {
    ...
    it("TC03_Geçerli kullanıcı ile login olmalı", () => {
        cy.fixture("LoginInfo").then((expected) => {
            LoginPage.visit();
            LoginPage.openLoginPopup()
    ...
    it("TC04_Geçersiz kullanıcı ile login olmamalı", () => {
        cy.fixture("LoginInfo").then((expected) => {
            LoginPage.visit();
            LoginPage.openLoginPopup()
     ...
     it("TC05_Geçersiz eposta formatı ile login", () => {
        cy.fixture("LoginInfo").then((expected) => {
            LoginPage.visit();
            LoginPage.openLoginPopup()
            LoginPage.fillEmail(expected.invalidUser.email);
     ...
     it("TC06_Form alanlarının boş bırakılıp login olunması", () => {
        cy.fixture("LoginInfo").then((expected) => {
            LoginPage.visit();
            LoginPage.openLoginPopup()
            LoginPage.fillEmail(expected.invalidUser.email);
    ...
    it("TC07_Üst üste başarısız login sonrası banned", () => {
        cy.fixture("LoginInfo").then((expected) => {
            LoginPage.visit();
            LoginPage.openLoginPopup()
            LoginPage.fillEmail(expected.invalidUser.email);
    ...
    it("TC08_Şifremi Unuttum ve Yönlendirme Sayfası", () => {
        cy.fixture("LoginInfo").then((expected) => {
            LoginPage.visit();
            LoginPage.openLoginPopup();
